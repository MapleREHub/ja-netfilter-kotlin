...
# 该脚本会：
#   1. 检测当前操作系统
#   2. 定位 JetBrains 各产品的 vmoptions 文件
#   3. 添加 -javaagent:ja-netfilter.jar=jetbrains 到每个产品的 vmoptions
#   4. 设置环境变量，使 vmoptions 生效
#
# 使用方法：
#   chmod +x install.sh
#   ./install.sh
...
if [ "$OS_NAME" = "Darwin" ]; then
  echo 'done. the "killall Dock" command can fix the crash issue.'
else
